					...
					<div class="password-wrapper">
						<div class="inputter__flex">
							<button v-for="(item, index) in passwdArray" :key="index" class="num-button__flex spread-effect fantasy-font__2_3rem">
								<button v-if="index === 9" class="num-button__flex spread-effect fantasy-font__2_3rem">{{ checkInvisibleNum(item) }}</button>
								<div v-else @click="setPasswd(item)">{{ item }}</div>
							</button>
							<button class="num-button__flex spread-effect fantasy-font__2_3rem">{{ tempNum }}</button>
